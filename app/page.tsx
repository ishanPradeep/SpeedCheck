          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              <div className="relative group">
                <div className={`p-2 sm:p-3 rounded-xl backdrop-blur-sm border transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-white/10 border-white/20 group-hover:bg-white/20' 
                    : 'bg-white/60 border-gray-200 group-hover:bg-white/80'
                }`}>
                <Wifi className={`h-6 w-6 sm:h-8 sm:w-8 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'} transition-colors duration-300`} />
              ...
              <div className="min-w-0 flex-1">
                <h1 className={`text-lg sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r ${
                  isDarkMode 
                    ? 'from-blue-400 to-cyan-400' 
                    : 'from-blue-600 to-cyan-600'
                } bg-clip-text text-transparent transition-all duration-300 truncate`}>
                  GlobalSpeedTrack
                </h1>
                <div className="flex items-center space-x-2">
                  <StatusIcon className={`h-3 w-3 sm:h-4 sm:w-4 ${connectionStatus.color}`} />
                  <p className={`text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} transition-colors duration-300 truncate`}>
            ...
            <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
              ...
              <div className={`flex items-center space-x-2 lg:space-x-4 text-sm ${
                ...
                <div className={`flex items-center space-x-2 backdrop-blur-sm rounded-xl px-3 py-2 border transition-all duration-300 ${
                  ...
                  <span className="font-medium hidden lg:inline">{userInfo?.city || 'Detecting...'}</span>
                  <span className="font-medium lg:hidden">{userInfo?.city?.split(',')[0] || 'Detecting...'}</span>
                </div>
                <div className={`flex items-center space-x-2 backdrop-blur-sm rounded-xl px-3 py-2 border transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-white/10 border-white/20 hover:bg-white/20' 
                  ...
                  <span className="font-medium hidden xl:inline">{userInfo?.isp || 'Detecting ISP...'}</span>
                  <span className="font-medium xl:hidden">{userInfo?.isp?.split(' ')[0] || 'ISP...'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="md:hidden flex items-center space-x-2">
              ...
              <Button
                ...
                className={`p-2 ${isDarkMode ? 'text-white hover:bg-white/10' : 'text-gray-700 hover:bg-gray-100'}`}
              >
      ...
      <div className="container mx-auto px-4 py-4 sm:py-6 lg:py-8">
        <Tabs defaultValue="test" className="space-y-4 sm:space-y-6 lg:space-y-8">
          <TabsList className={`grid w-full grid-cols-3 backdrop-blur-xl border transition-all duration-300 ${
            ...
            <TabsTrigger 
              value="test" 
              className={`transition-all duration-300 font-medium text-xs sm:text-sm lg:text-base ${
                isDarkMode 
            ...
            >
              ...
              <span className="hidden sm:inline">Network Quality</span>
          ...
          <TabsContent value="test" className="space-y-4 sm:space-y-6 lg:space-y-8">
            {/* Enhanced Main Speed Test - Responsive */}
                ...
                <div className="text-center space-y-6 sm:space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 max-w-5xl mx-auto">
                    ...
                    <div className="space-y-4 sm:space-y-6">
                      ...
                      <div className="text-center">
                        <div className={`text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent`}>
                          {results.downloadSpeed.toFixed(1)}
                        </div>
                        <div className={`text-sm sm:text-lg font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Mbps</div>
                      </div>
                    </div>
                  ...
                  {isRunning && (
                    <div className="space-y-3 sm:space-y-4 max-w-2xl mx-auto">
                      <div className={`backdrop-blur-sm rounded-xl p-4 sm:p-6 border ${
                        isDarkMode 
                      ...
                      />
                        <div className={`text-center mt-2 sm:mt-3 text-sm sm:text-lg font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        {getProgressText()}
                        </div>
                        <div className={`text-center text-xs sm:text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  ...
                  <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
                    ...
                    {(results.downloadSpeed > 0 || results.uploadSpeed > 0) && !isRunning && (
                      <>
                        ...
                        >
                          <Share2 className="h-5 w-5 sm:h-6 sm:w-6 mr-2 sm:mr-3" />
                          Share Results
                        </Button>
                  ...
                  {/* Enhanced Result Summary - Responsive */}
                  {!isRunning && (results.downloadSpeed > 0 || results.uploadSpeed > 0) && (
                    <div className={`rounded-2xl p-4 sm:p-6 lg:p-8 border backdrop-blur-xl transition-all duration-500 ${
                      isDarkMode 
              ...
              <CardContent className="p-4 sm:p-6 lg:p-8">
                <div className="flex items-center space-x-2 sm:space-x-3 mb-4 sm:mb-6">
                  <div className={`p-2 sm:p-3 rounded-xl backdrop-blur-sm border ${
                    ...
                    <Info className={`h-5 w-5 sm:h-6 sm:w-6 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                  </div>
